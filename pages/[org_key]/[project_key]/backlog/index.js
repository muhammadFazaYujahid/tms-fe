...
const Backlog = ({ myQuery }) => {
    ...
    const { contentDialog, setContentDialog, setOrgKeyLink, setProjectKeyLink } = useContext(LayoutContext);
    ...
    useEffect(() => {
        ...
        sprintService.getSprint(sessionStorage.getItem('project_key')).then((data) => { setSprints(data.sprint) });
    ...
    const createSprint = () => {
        const sprintService = new SprintService();
        sprintService.createSprint(Cookies.get('project_key')).then(() => {
            emit('refreshBacklog');
        })
    }
    ...
    const deleteSprint = (sprint, backlog_key) => {
        ...
        const sprintService = new SprintService();
        sprintService.deleteSprint(data)
            .then((res) => {
                let summaryNotif = '';
                let detailNotif = '';
                let severity = '';
    ...
    const onDragEnd = result => {
        ...
        const tastService = new TaskServices();
        tastService.changeSprint(data);
        emit('refreshBacklog');
    ...
    const sprintButton = (sprint, backlog_key) => {
        ...
        if (sprint.status == 'not_start') {
            ...
            return (
                <SplitButton className='p-button-outlined p-0 p-button-sm' size='small' label="Finish Sprint" model={sprintSetting(sprint, backlog_key)} style={{ borderRadius: "0px" }} />
            )
        } else {
                    ...
                    <Button onClick={(e) => {
                        ...
                        sprintService.startSprint(sprint_key)
                            .then((res) => {
                                // emit('refreshBacklog');
                                if (res.success) {
                                    Router.replace('/' + sessionStorage.getItem('org_key') + '/' + sessionStorage.getItem('project_key') + '/board')
                                }
                            })
